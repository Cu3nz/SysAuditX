...
def generar_contenido_txt():
    ...
    comando_netdiscover = f"sudo netdiscover -i {interfaz} -r {ip_detectada} -P -c 5"
    resultado_netdiscover = ejecutar_comando(comando_netdiscover)
    contenido.append(f"\n==================== HOSTS DESCUBIERTOS EN LA RED ====================\n{resultado_netdiscover}\n")

    ips_descubiertas = []
    for linea in resultado_netdiscover.splitlines():
        coincidencia = re.search(r'(\d{1,3}(?:\.\d{1,3}){3})', linea)
        if coincidencia:
            ip = coincidencia.group(1)
            if ip != ip_detectada:  # evitar la IP local, opcional
                ips_descubiertas.append(ip)

        # Lista de puertos peligrosos y comunes en PYMEs
    puertos_peligrosos = "21,22,23,25,53,80,110,139,143,443,445,3306,3389,5900,8080"
    # Escaneo completo con Nmap para todas las IPs detectadas
    for ip in ips_descubiertas:
        log(f"ESCANEO DE SERVICIOS EN {ip}", f"sudo nmap -sV -T4 -Pn -n --host-timeout 30s -p {puertos_peligrosos} {ip}")
